...
from src.ml_project.modeling.predict import ModelSchemaMismatchError, predict
...
class PredictRequest(BaseModel):
    ...
    instances: list[InferenceInstance] = Field(min_length=1, max_length=MAX_PREDICTION_INSTANCES)
...
def resolve_api_model_version() -> str:
    """Retorna a versao governada ou o run associado ao artefato local."""
    if MODEL_METADATA_PATH.exists():
        metadata = json.loads(MODEL_METADATA_PATH.read_text(encoding="utf-8"))
        registry_info = metadata.get("model_registry") or {}
        use_registry = os.getenv("MLFLOW_USE_MODEL_REGISTRY_FOR_INFERENCE", "false").lower() in {
            "1", "true", "yes"
        }
        if (
            use_registry
            and registry_info.get("registered_model_name")
            and registry_info.get("version")
        ):
...
def create_app() -> FastAPI:
    ...
    async def log_request(request: Request, call_next):
        ...
        try:
            ...
            record_operational_metric(
                "http_request", request_id=request_id, method=request.method,
                path=request.url.path, status_code=500, latency_ms=round(elapsed_ms, 3),
            )
            logger.exception(
    ...
    async def validation_exception_handler(_request: Request, exc: RequestValidationError):
        return build_error_response(
            ...
            "Payload de inferencia invalido.",
            exc.errors(),
            422,
        )
    ...
    async def model_not_found_handler(_request: Request, exc: FileNotFoundError):
        return build_error_response(
            "MODEL_NOT_FOUND", "Artefato de modelo ou metadata nao encontrado.", [str(exc)], 503
        )

    @app.exception_handler(ModelSchemaMismatchError)
    async def model_schema_mismatch_handler(_request: Request, exc: ModelSchemaMismatchError):
        return build_error_response("MODEL_SCHEMA_MISMATCH", str(exc), http_status=500)

    @app.exception_handler(ValueError)
    async def prediction_value_error_handler(_request: Request, exc: ValueError):
        return build_error_response("INVALID_INPUT_SCHEMA", str(exc))
    ...
    async def unexpected_error_handler(_request: Request, exc: Exception):
        ...
        return build_error_response(
            "PREDICTION_FAILED",
            "Falha inesperada durante a inferencia.",
            http_status=500,
        )
    ...
    def health() -> HealthResponse:
        ...
        metadata_available = MODEL_METADATA_PATH.exists() and PROCESSED_METADATA_PATH.exists()
        preprocessor_available = MODEL_PREPROCESSOR_PATH.exists()
        return HealthResponse(
            status=(
                "ok"
                if model_available and metadata_available and preprocessor_available
                else "degraded"
            ),
    ...
    def predict_endpoint(payload: PredictRequest) -> PredictResponse:
        ...
        record_operational_metric(
            "prediction", instances=len(prediction_items), model_version=model_version,
